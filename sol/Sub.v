...
(** What is the _smallest_ type [T] that makes the following
    ...
    true, but there is no smallest one of these: [Top->A->A] and [Top->B->B]
    are both solutions, but they have no common subtype. []
*)
...
Is this a good idea? Briefly explain why or why not.
  ...
  Answer: No, since it will break preservation: [(true,unit).1] has
  type [Unit] according to this rule, but reduces to [true], which
...
(* SOLUTION: *)
  TProd String (TProd TTop Float).
Definition Employee : ty := 
(* SOLUTION: *)
  TProd String (TProd Integer TTop).
...
Example typing_example_0 : 
  empty |- 
           (tpair (tabs z A (tvar z)) 
                    (tabs z B (tvar z))) \in 
                               
           (TProd (TArrow  A A) (TArrow B B)).
Proof.
  auto.
Qed.
(** [] *)
...
Example typing_example_1 : 
  empty |- 
           (tapp (tabs x (TProd TTop (TArrow B B))
                             (tsnd (tvar x)))
                   (tpair (tabs z A (tvar z)) 
                            (tabs z B (tvar z)))) \in
           (TArrow B B).
...
Proof with auto.
  ...
  Case "S_Trans". subst. assert (U = TBool); subst...
Qed.
...
Proof with eauto.
  ...
  (* SOLUTION: *)
  subtype_cases (induction Hs) Case; subst; intros; try solve by inversion.
    Case "S_Refl".
      exists V1. exists V2. subst...
    Case "S_Trans".
      apply IHHs2 in HeqV. inversion HeqV as [U1 [U2 [HeqS [HU1 HU2]]]].
      apply IHHs1 in HeqS. inversion HeqS as [S1 [S2 [HeqS1 [HS1 HS2]]]].
      exists S1. exists S2. subst...
    Case "S_Arrow".
      exists S1. exists S2. inversion HeqV; subst...  Qed.
...
Proof with eauto.
  intros S T1 T2 Hsub. remember (TProd T1 T2) as TProd.
  generalize dependent T2. revert T1.
  subtype_cases (induction Hsub) Case; intros;
    try solve by inversion.
  Case "S_Refl".
    subst. subst. exists T1. exists T2...
  Case "S_Trans".
    destruct (IHHsub2 T1 T2) as [S1 [S2 [HeqUprod [S1subT1 S2subT2]]]]...
    destruct (IHHsub1 S1 S2) as [U1 [U2 [HeqSprod [U1subS1 U2subS2]]]]...
    exists U1. exists U2...
  Case "S_Prod".
    inversion HeqTProd. subst.
    exists S1. exists S2...
Qed.
(** [] *)
...
Proof with eauto.
  ...
  remember (TProd T1 T2) as TProd. generalize dependent T2. revert T1.
  has_type_cases (induction Htyp) Case; intros; try solve by inversion.
  Case "T_Pair".
    exists t1. exists t2...
  Case "T_Sub".
    rewrite HeqTProd in H. apply sub_inversion_prod in H.
    inversion H as [S1 [S2 [HeqSprod [_ _]]]].
    destruct (IHHtyp Hval S1 S2) as [t1 [t2 Heqtpair]]...
Qed.
...
Proof with eauto.
  ...
  Case "T_Fst".
    ...
    SCase "t is a value".
      ...
      inversion Ht as [t1 [t2 teq]]. subst.
...
Proof with eauto.
  intros Gamma t1 t2 T Htyp.  remember (tpair t1 t2) as tpair.
  has_type_cases (induction Htyp) Case;
    inversion Heqtpair; subst; intros...
  Case "T_Sub".
    destruct IHHtyp as [T1 [T2 [Hsub [Htyp1 Htyp2]]]]...
    exists T1. exists T2...
Qed.
...
Lemma typing_inversion_fst : forall Gamma t T,
  Gamma |- (tfst t) \in T ->
  exists T1, exists T2,
    T1 <: T /\ Gamma |- t \in (TProd T1 T2).
Proof with eauto.
  intros Gamma t T Htyp. remember (tfst t) as tfst.
  has_type_cases (induction Htyp) Case;
    inversion Heqtfst; subst; intros...
  Case "T_Sub".
    destruct IHHtyp as [T1 [T2 [Hsub Htyp1]]]...
Qed.

Lemma typing_inversion_snd : forall Gamma t T,
  Gamma |- (tsnd t) \in T ->
  exists T1, exists T2,
    T2 <: T /\ Gamma |- t \in (TProd T1 T2).
Proof with eauto.
  intros Gamma t T Htyp. remember (tsnd t) as tsnd.
  has_type_cases (induction Htyp) Case;
...
(** Each part of this problem suggests a different way of
    ...
    - Suppose we add the following reduction rule:
      ...
      Answer: Preservation fails.  For example, [unit] has type
      [Unit] but steps to [(\x:Top. x)], which does not have type
      [Unit].
    - Suppose we add the following subtyping rule:
                               --------------                        (S_Funny3)
                               Unit <: Top->Top

      Answer: Progress fails.  For example, [unit (\x:Top.Top)] is
    ...
    - Suppose we add the same evaluation rule _and_ a new typing rule:
      ...
      Answer: Preservation fails. For example, [unit (\x:A,x)] has
      type [Top], but it steps to [(\x:A,x) unit], which is ill
      typed.
    - Suppose we _change_ the arrow subtyping rule to:
      ...
      Answer: Preservation fails.  For example, [(\x:Unit*Unit. x.fst)
      unit] has type [Unit], but steps to [unit.fst] which is ill
      typed. (In order to type [(\x:Unit*Unit. x.fst) unit] we use
      [T_Sub] twice; once to give [unit] the type [Top], and once to
      give [\x:Unit*Unit. x.fst] the type [Top -> Unit] using
      [S_Arrow']).
[]
*) 
