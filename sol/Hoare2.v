...
The second use of consequence is trivial to justify, while the first
...
Proof.
  ...
  Case "invariant preserved by loop".
    eapply hoare_consequence_pre. apply hoare_asgn.
    unfold assert_implies. intros.
    unfold bassn, assn_sub, beval, aeval in *.
    rewrite update_eq. destruct H as [H1 H2].
    apply ble_nat_true in H2. rewrite <- H1.
    apply parity_ge_2. assumption.
  Case "invariant holds initially".
    unfold assert_implies. intros st H. subst. reflexivity.
  Case "invariant holds at the end".
    unfold assert_implies. intros st H. destruct H as [H1 H2].
    unfold bassn, assn_sub, beval, aeval in *.
    rewrite not_true_iff_false in H2. apply ble_nat_false in H2.
    rewrite <- H1. symmetry. apply parity_lt_2. assumption.
  Qed.
  (** [] *)
...
Proof.
  ...
  unfold is_wp. split.
  Case "valid triple".
    (* The Hoare rules make proving this first conjunct
       more convenient (in this case it doesn't matter). *)
    unfold hoare_triple. intros. inversion H. subst. simpl in *.
    rewrite update_eq. omega.
  Case "weakest precondition".
    unfold assert_implies. intros. unfold hoare_triple in *.
    apply H with (st' := update st X (st Y + 1)) in H0;
      [| apply E_Ass; reflexivity].
    rewrite update_eq in H0. omega. Qed.
(** [] *)
...
Proof.
(* SOLUTION: *)
  unfold hoare_triple. unfold havoc_pre. unfold assert_implies. intros.
  apply H with st. constructor. assumption. Qed.
End Himp2.
(** [] *)
...
Proof. (* SOLUTION: *) intros. verify. Qed. (** [] *)
...
Proof.
  intros. destruct m. apply ex_falso_quodlibet. apply H. reflexivity.
  simpl. rewrite <- minus_n_O. reflexivity.
Qed.
